...
(function($) {
	...
	$.fn.aapf = function(){
		...
		function carregarLayout(data){
			...
			/*if(!optDadosCliente.dadosCliente.suporteVersaoNavegador){
				alert("não suporta navegador");
			}*/
			...
			if(data.dataUltimoAcesso){
				$(".ultmio-acesso").html("Último acesso: " +data.dataUltimoAcesso+" "+data.horaUltimoAcesso);
			}
			...
			if(ambiente === "naocorrentista"){
				...
				var textoAgConta = "Agência: " + data.agencia + " " + "Conta: " + data.conta;
				...
				if(optDadosCliente.dadosCliente.indicadorImagem === "S"){
					$.getFoto({"id" : data.codigo,"tipo" :"srp"},function(foto) {
						if(foto){
			...
			if(optDadosCliente.dadosCliente.ativarIBT){
				$.carregarIBT(optDadosCliente.dadosCliente.servidor);
			}
			
			$.verificarPublicoAlvoMensagemInstantanea(function(data){
				$(".menu-relacionamento-barra .botaoTelefone").addClass("botaoChat");
				$(".menu-relacionamento-barra .botaoTelefone a").attr("title","Fale com o seu gerente").attr("alt","Fale com seu gerente");
	...
	/***************************************************************************
	 * 
	 * Plugin responsável pelo controle das contas do cliente;
	 * 
	 * 
	 * 
	 */	
	...
	/***************************************************************************
	 * 
	 * Plugin com de cronometro da sessão
	 * 
	 * options : tempo {tempo: ?}
	 * 
	 * 
    	...
    	contadorSessao: function(options) {
				...
				verificaSessaoAtiva(function(retorno,ignorarVerificaCookie){
					...
					if(retorno){
						...
						if(tempoSessao.getMinutes() ===  0 && tempoSessao.getSeconds() === 0){
							...
							clearTimeout(timeout);
							ligado = false;
							optSessao.obj.text("Sessão expirada!");
						}else{
							if(ligado){
								optSessao.obj.text("Sessão " +  tempoSessao.format("MM'min'ss"));
						...
						if(!sessaoExpirou){
							sessaoExpirou = true;
							pararCronometro();
							optSessao.obj.text("Sessão finalizada!");
			...
			function criarTelaInformacaoSessao(modal){
				...
				var ul = $("<ul></ul>",{"class":"tela-expira-sessao"})
					.append($("<li></li>",{"class":"tela-expira-sessao-texto"}).text("Sua sessão expira em"))
					...
					.append($("<li></li>",{"class":"tela-expira-sessao-botoes"})
							.append(
									$("<input></input>",{"class":"botaoapf","type":"button","id":"botaoRevalidaSessao", "title":"Revalidar sessão"}).val("REVALIDAR SESSÃO")
									.click(function(e){
			...
			function criarTelaRefazerlogin(modal){
				...
				.append($("<li></li>",{"class":"tela-expira-sessao-texto"}).html("Sua sessão expirou."))
			...
			function verificaSessaoAtiva(funcaoCallback,ignorarVerificacaoCookie){//verifica se o finalizou a sessão ou limpou os cookies
				
				var session1 = getCookie("aapf.logado");
	...
	$.fn.acheFacil = function(options) {
									  ...
									  .append($("<a></a>",{"tabindex" : "-1","url" : transacao.url, "tipoLink" : transacao.tipoLink})
											  ...
											  .click(function(e) {
											   ...
											   .keyup(function(event) {
											   		if (event.keyCode === 27) { // tecla esc fecha a lista de transações
											   			$("#listaAcheFacil").slideUp();
													} else if (event.keyCode === 13) { // tecla enter fecha a lista e direciona para transação selecionada
														$("#listaAcheFacil").slideUp();
														opt.obj.val("");
														if($(this).attr("tipoLink") == "externo"){
															window.open($(this).attr("url"));
		...
		function buscaTexto(texto, textoBusca){
			var coordenadasPalavra = [];
			var palavras = textoBusca.split(" ");
			var nomeAmigavel = $.acentuacaoAmigavel(texto);
			var palavraMenu = ""; 
			if(nomeAmigavel!=null){
				palavraMenu = nomeAmigavel.toUpperCase();
			}
			if(palavras !=null && palavras.length > 0){
				for(k=0;k<palavras.length;k++){
					var palavraBusca = palavras[k];
					var indice = palavraMenu.indexOf(palavraBusca);
					if(indice === -1){		
						return(false);
					}
				}
	...
	$.fn.toolbarMenuLateral = function(options) {
		...
		//Função que atualiza as notificações de solicitação de relacionamento e novas mensagens
	...
	$.fn.menuCompleto = function(options) {
		...
		// Cria a lista de menus no 1º nivel
		function criarMenu() {
			...
			if(scrollTop > 0 ){
				if(alturaMenuLateral < $(".corpo").outerHeight(true)){
					if(alturaMenuLateral < $(window).outerHeight(true)){
						...
						if(direcao === 1){
							if((alturaMenuLateral - $(window).outerHeight(true)) < scrollTop){ // verifica se o scrolltop + a altura do menu-lateral é maior que a altura da tela
								menuLateral.css({"position":"fixed","top":-(alturaMenuLateral - $(window).outerHeight(true)) + "px"});//posiciona o menu-lateral na altura do scrolltop
							}
						}else{
							if((alturaMenuLateral - $(window).outerHeight(true)) > scrollTop){ // verifica se o scrolltop + a altura do menu-lateral é maior que a altura da tela
								menuLateral.css({"position":"relative","top":"0px"});//posiciona o menu-lateral na altura do scrolltop
							}
						}
					}
		...
		function criarMeuMenu(menuNivel1){
			...
			// Monta a lista de transações favoritas
		...
		function criarListaMenuPersonalizado(ul){
			...
			if(ul.children().length === 0 ){
				$("<li></li>").html("Não há menus personalizados").appendTo(ul.addClass("sem-informacao"));
			}			
		}
		function criarMenuFavoritos(div){

			var divScroll = undefined;
			$("<div></div>",{"class":"meu-menu-titulo"}).html("Transações favoritas").appendTo(div);
			$("<div></div>",{"class":"meu-menu-mensagem-erro"}).appendTo(div);
			ul = $("<ul></ul>").appendTo(
		...
		function criarListaFavoritos(ul){
			...
			if(optDadosCliente.dadosCliente.favoritos && optDadosCliente.dadosCliente.favoritos.length > 0){
				...
				$("<li></li>").html("Não há transações favoritas").appendTo(ul.addClass("sem-informacao"));
			}				
		...
		//abrir tela de personalização do menu
	...
	$.fn.menuRelacionamento = function(options) {
		...
		// função que filtra os relacionamentos pela parte nome digitada
		...
		// Função que cria a lista de relacionamentos
		function criarMenuRelacionamentos() {
			...
			if(optDadosCliente.dadosCliente.relacionamentos && optDadosCliente.dadosCliente.relacionamentos.length > 0){
				...
				ulItens.append($("<li></li>",{"class":"sem-informacao"}).text("Não há relacionamentos."));
			}
		...
		function adicionarElementoMenuRelacionamento(ulItens,objeto){   
			...
			.click(function(e){ // mostra o menu de transações
			...
			if(objeto.dependenciaRelacionamento){
				...
				objetoFoto = {"id": objeto.identificadorConta,"tipo" : "srp","hasFoto" : objeto.indicadorFoto};
			}else{
				objetoFoto = {"id": "FB" + objeto.uid,"tipo" : "fb","pic_square" : objeto.pic_square};
			}
		...
		function criarMenuTranscacoesRelacionamento(elemento, objeto){
			...
			var transacoes = [{"nome" : "Transferência para conta corrente","url" : "/aapf/transferencia/818-00.jsp","transacao" : 1},
			                  {"nome" : "Transferência para poupança","url" : "/aapf/transferencia/819.jsp","transacao" : 2},
			                  {"nome" : "Últimas transações","url" : "/aapf/comprovante/869-CO.jsp","transacao" :3}];
		...
		function executarTransacao(objeto,transacao){
			...
			var parametros = {};
			if(objeto.dependenciaRelacionamento){
				parametros = {"dependenciaDestino" : objeto.dependenciaRelacionamento,"numeroContratoDestino" : objeto.numeroContratoRelacionamento};
			}else{
				parametros = {"codigoRelacionamentoExterno" : objeto.uid};
			}
			var url = transacao.url;
			if(transacao.transacao === 2){// transferência de conta corrente para poupança
				parametros["codT"] = "1"; 
			}else if(transacao.transacao === 3){// lista de comprovantes de transferências para o relacionamento
				parametros["codT"] = "1";
		...
		// busca as solicitações de relacionamento enviadas e recebidas
		...
		//Envia a resposta da solicitação
		...
		// verifica se não há solicitações e esconde o elemento de solicitações
	...
	$.fn.menuRelacionamentoPJ = function(options) {
		...
		function criarMenu() {
			...
			if(opt.convenios && opt.convenios.retorno != 1 &&  opt.convenios.length > 0){
				...
				ulItensMenu.append($("<li></li>",{"class":"sem-informacao"}).text("Não há relacionamentos com empresas."));
			}
		}
		function criarSubMenu(elementoPai,objeto){
			...
			$.each(objeto.lancamentos,function(i, lancamento) {
				...
				.appendTo($("<li></li>").appendTo(ulItens))
				.click(function(e){
					//fecha o su menu e carrega a transação e comprovante
					fecharSubMenu();
	...
	/***************************************************************************
	 * 
	 * Plugin que cria o menu de telefones úteis
	 * 
	 **************************************************************************/	
	$.fn.menuTelefones = function(options) {
		...
		function criarListaTelefones() {
			...
			if(optDadosCliente.dadosCliente.agencia){
				$("<li></li>").html("<b>Agência:</b> "+optDadosCliente.dadosCliente.agencia+ "&nbsp;-&nbsp;"+optDadosCliente.dadosCliente.nomeAgencia).appendTo(itens);
				if(optDadosCliente.dadosCliente.telefoneAgencia){
					$("<li></li>").html("<b>Telefone:</b> ("+optDadosCliente.dadosCliente.dddAgencia+ ")"+optDadosCliente.dadosCliente.telefoneAgencia).appendTo(itens);
				}
				if(optDadosCliente.dadosCliente.nomeGerente){
					...
					if(optDadosCliente.dadosCliente.telefoneGerente){
						$("<li></li>").html("<b>Telefone:</b> ("+optDadosCliente.dadosCliente.dddGerente+ ")"+optDadosCliente.dadosCliente.telefoneGerente).appendTo(itens);
					}					
				}
			}
		...
		function montarItemFaleComGerente(){
			...
			$("<a></a>",{"href":"javascript:void(0)"}).html("Fale com o seu gerente").appendTo(item)
			.click(function (e){
				$.carregaFormularioAjax("/aapf/relacionamento/mensagemInstantanea.jsp",{});
			});
		}
	}
	...
	$.fn.menuMensagens = function(options) {
		...
		function criarMenu() {
			...
			try {
				if(opt.mensagens && opt.mensagens.length > 0){
					...
					ulItens.append($("<li></li>",{"class":"sem-informacao"}).text("Não há mensagens."));
				}
			} catch (err) {
				console.log(err);
			}
					...
					if(retorno){
						...
						$.ajaxServico({"nomeServico":"excluirComunicado",
							...
							"funcaoErro" : function(data) {
								...
								setTimeout(function(){
									eleMentoMensagemErro.slideUp(function(){ // fechar o elemento com o efeito slide e depois exclui o elemento após dois segundos
	...
	/* funcão que troca a opção do menu lateral com o efeito de slide */
	...
	/*
	 * 
	 * Plugin que faz a navegação do tour do sistema
	 * 
	 */
	...
	$.fn.tourapf = function(options){
		...
		function limparElementos(){ // verifica se os elementos informados existem, se não existirem serão descartados da lista
		...
		function criarElementosNavegacao(){
			...
			$("<a></a>",{"class": "navegacao-fechar" ,"href":"javascript:void(0);","title":"Finalizar tour do auto atendimento pessoa física"}).html("X")
			.click(function(e){
				apresentacaoFechar();
			})
			.appendTo(div);
...
/*
 * Cria a caixa de dialogo com o conteúdo de ajuda da transação
 * 
 */
...
/*
 * 
 * Criar menu de seleção do tipo de arquivo que deseja salvar
 */
$.criarCaixaDialogoSalvar = function(botao,event){
	...
	// Descrição dos tipos de arquivos passíveis de exportação
	
	var descricaoTipos = {"pdf": "PDF (pdf)","txt" : "Arquivo texto (txt)","csv" : "Texto tabulado (csv)","ofx" : "Money 2000+ (ofx)","ofc" : "Money 97 à 99 (ofc)" };
	var tiposTransacao = [];
						...
						.click(function(e){
							$(".menu-completo").menuCompleto("salvarFavorita",$("#nomePersonalizadoFavorita").val(),
									function(){
										...
										$(".caixa-dialogo .caixa-dialogo-conteudo").append($("<div></div>",{"class":"trancasao-sucesso"}).html("Transação favorita cadastrada com sucesso!"));
										setTimeout(function(){
											$(".caixa-dialogo").remove();
										},1500);
									},
...
/* verifica se o item de menu está na lista de menus personalizados */
...
/*
 * função que chama a página que responsável por salvar os documentos nos
 * diversos formatos
 * 
 */
$.salvarDocumento = function (dados){
	var formTemp = $('<form></form>').attr("id",'hiddenForm' ).attr("name", 'hiddenForm').attr("action","/aapf/includes/salvarDocumento.jsp").attr("target","_self").attr("method","POST");
...
/*
 * 
 * função responsável pela criação do modal parametros opcoes : { url : "página
...
/*
 * Carrega o formulario da transação através do menu url :
 */
...
/*
 * 
 * função que executa chamada no servidor para reiniciar sessão
 * 
 */
...
$.trocarConta = function(dados){
	$.abrirModal({"url":"/aapf/seguranca/loginTrocaConta.jsp","parametros" : dados,"width":"720","height":"390","resizeAuto":false});
}

// Chamada do serviço de troca de titular
...
/*******************************************************************************
 * ***********************************************************************
 * Funções de tratamento do cache das imagens dos relacionamentos
 * **********************************************************************
 ******************************************************************************/
...
/* Cria a estrutura de cache que irá guardar as fotos dos relacionamentos */
$.incializadb = function (funcao,funcaoNaoSuportaIDB){
	if(window.indexedDB){
		if(!db){
		
			request = window.indexedDB.open("fotosaapf2", 1);
...
$.atualizaDadosPerfilLocal = function(dados){
	...
	$.novaFoto({"id" :""+ dados.id , "base64" : dados.base64,"dataAtualizacao" : dados.dataAtualizacao});
}
...
$.getFoto = function(objeto,funcao){
	if(window.indexedDB){
		$.incializadb(function(){
		    var request = db.transaction(["fotos"], "readwrite").objectStore("fotos").get(""+objeto.id);
		    request.onsuccess = function(event) {
		    	if(event.target.result){
		    		if(objeto.tipo === "srp" && $.verificaAtualizacaoFoto(objeto.id,event.target.result.dataAtualizacao)){
		    			$.buscaFotoServidor(objeto,function(data){
		    				$.atualizaFotoLocalRelacionamento({"id" :""+objeto.id , "base64" : data.base64});
		    				funcao(data.base64);							  
		    			});
		    		}else{
...
/* Verifica se a foto do cache necessita de atualização */
$.verificaAtualizacaoFoto = function (id,data){
	if(parseInt(optDadosCliente.dadosCliente.codigo) === parseInt(id)){
		if(optDadosCliente.dadosCliente.indicadorImagem === "S"){
			if(data === "" && data != optDadosCliente.dadosCliente.dtAtuaFoto){
				return(true);
			}
		}
	}else{
...
/*******************************************************************************
 * fim das funções de tratamento das fotos dos relacionamentos
 ******************************************************************************/
/*
 * 
 * Funções de conexão com facebook
 * 
 */

$.verificaLoginFB = function(){
	if(optDadosCliente.dadosCliente.uid){
		var url = "https://www.facebook.com/dialog/oauth?redirect_uri="+ urlBase +"/aapf/srp/relacionamentoPF/verificarLogadoRedeRelacionamento.jsp?uid="+optDadosCliente.dadosCliente.uid+"&client_id=380699942014883&response_type=token&format=jsonp";
...
$.montarUrl = function(comando){
	var url = undefined;
	if(comando === "login"){
		 url = "https://www.facebook.com/dialog/oauth?client_id=380699942014883&&scope=public_profile%2Cuser_friends%2Cuser_about_me%2Cuser_location%2Cuser_events%2Cuser_likes%2Cuser_birthday%2Cemail%2Cuser_videos%2Cuser_photos&display=popup&response_type=token" +
...
$.getDadosCliente = function(token,funcao){
	$.ajaxApf({
		...
		parametros : {"q" : "SELECT uid,first_name,last_name FROM user WHERE is_app_user and uid = me()","access_token":token},
		"tiporetorno" :"jsonp",
...
$.getRelacionamentos = function(token,uid,funcao){
	$.ajaxApf({
		type : "get",
		url: "https://graph.facebook.com/fql",
		parametros : {"q" : "SELECT uid, name, pic_square FROM user WHERE is_app_user and  (uid IN (SELECT uid2 FROM friend WHERE uid1 = me())) order by name","access_token":token},
		"tiporetorno" :"jsonp",
		"funcaoSucesso" : funcao
	});
}
// Verifica se o cliente faz parte do publico alvo da funcionalidade de mensagens instantaneas
$.verificarPublicoAlvoMensagemInstantanea = function(funcao){
	$.ajaxServico({"nomeServico":"verificarPublicoAlvoMensagemInstantanea",
		"funcaoOk" : function(data) {
			if(data.dadosEncarteiramento.length > 0 ){
				funcao(data.dadosEncarteiramento[0]);
			}
		},
		"funcaoErro" : function(data) {
...
$.atualizaRelacionamentos = function (token,uid,janela){
	janela.$.getRelacionamentos(token,uid,function(data){
		...
		janela.$.each(data.data,function(j,objetoFB){
			var bAchou = false;
			for(i=0;i<relacionamentosBB.length;i++){
				var objetoBB = relacionamentosBB[i];
				if(parseInt(objetoBB.uid) === parseInt(objetoFB.uid)){
					bAchou = true;
				}	
			}
			if(!bAchou){
				janela.optDadosCliente.dadosCliente.relacionamentos.push({"uid":objetoFB.uid,  "nome" : objetoFB.name,"pic_square" : objetoFB.pic_square,"tipo" : "fb"});
			}
		});
...
/* remove adesão do cliente ao aplicativo do banco no facebook no SRP */
...
/* Envia solicitação de relacionamento 
...
function enviarSolicitacao(solicitacao){
	$.ajaxServico({"nomeServico":"solicitarRelacionamento",
		...
		"funcaoOk" : function(data) {
			$(".caixa-dialogo .caixa-dialogo-conteudo").append($("<div></div>",{"class":"trancasao-sucesso"}).html("Solicitação enviada com sucesso!"));
			setTimeout(function(){
				$(".caixa-dialogo").remove();
...
/*******************************************************************************
 * fim das funções de conexão com o facebook /
 ******************************************************************************/
 ...
 * objeto : elemento que receberá o evento de scroll evento : objeto evento
 */
...
$.buscarBannerTransacao = function(){
	if($(".corpo #codigoBannerTransacao").val() !== 'null' || $(".corpo #codigoTransacao").val() === 'COCA'){
		var dados = {
				"url" : "/aapf/servico",
				"tiporetorno" : "json",
				"parametros" : {"servico" : "bannerTransacao", "codigoTransacao" : $(".corpo #codigoTransacao").val(), "codigoBannerTransacao" : $(".corpo #codigoBannerTransacao").val()},
				"verificaSessao" : true,
				"funcaoAntesExecucao" : function() {
				}
				,
	...
	if(href){
		...
		if(href.indexOf("javascript") === -1 && 
		   ...
		   href.lastIndexOf(".pdf") === -1){
			...
			if (href.indexOf("https://www2.bancobrasil.com.br/") != -1){
				...
				objetoLink.attr("href",uri + urlPart);
					...
					href.indexOf("bancobrasil.com.br") != -1){
				objetoLink.attr("href",href);
			}else if ((href.indexOf(uri) == -1) && (href.indexOf("javascript:") == -1) &&  (href.indexOf("/aapf/") == -1)){
				var click = "popupAlertaRedirecionamento('/aapf/direciona.jsp?url="+href+"','popUp','width=566,height=482');";
...
function criarApresentacao(dadosPublicador){
	...
	/*  Definição dos elementos que fazem parte do tour do sistema*/
	var itensApresentacaoCanvas = [
...
$.recuperarTicketCNL = function(){
		var dados = {
				"url" : "/aapf/includes/ticketCnl.jsp",
				"tiporetorno" : "json",
				"funcaoSucesso" : function(data) {
	            	try{
	            		if(data.erroCNL){
	            			console.log(data.erroCNL);
	            		}
