...
function limpaFormulario(objeto){
	...
	form.find("input[type='text'],textarea,select").each(
	        function() {
...
function getSenhaPlugin(objeto){
	if(objeto.tipoPlugin === "GBAS"){
		...
		try{
			if(GbPluginObj){
				GbPluginObj.Digest(objeto.numCode);
				GbPluginObj.Digest2(objeto.numCode);
...
//Permite a marcação do radio e alteração da cor da respectiva linha, ao
//clicar em qualquer área da "TR".
//Utiliza a variável global 'statusInicial', que é alterada na função
//"mudaCor".
...
function LimparMoeda(valor, validos) {
  ...
  function FormataData2(campo,teclapres){
	...
	function getSenhaLogin(div,nativo,idCh,idDg,idDg2,campo,seed,isIpad,isAndroid){
		try{
			...
			if(e == "ErroApplet"){
				...
				$("#divErros").html("<span style='color:red;'>Erro ao executar Login.<br/>Verifique se o plugin JAVA está ativado.</span>");				
				throw new e;
			}
		}
	}
	...
	/*
	 * Cria uma div por cima de outra deixando a que fica por trás
	 * sem poder ser modificada
	 * Parametros:
	 *		idDiv = div a ser bloqueada
	 *		textoBloqueio = texto que aparecerá quando a div for bloqueada		
	 */
	...
	function confirmar984(){
		if($("#senhaConta").val() === ''){
			$("#textoErro").html("Senha da conta corrente deve ser informada.");		
		}else{
			if($("#senhaConta").val().length < 6){
				$("#textoErro").html("O conteudo da senha da conta corrente deve ter 6 caracteres.");			
			}else{
				$.submeter($('#aapf')[0]);			
			}
		}
